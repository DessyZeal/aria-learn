...
export default function CQLevel2() {
  ...
  const [dragItems, setDragItems]         = useState<string[]>(['Beat 3', 'Beat 1', 'Beat 4', 'Beat 2'])
  const [arranged, setArranged]           = useState<string[]>([])
  const [activityDone, setActivityDone]   = useState(false)
  const [activityFeedback, setActivityFeedback] = useState<'correct' | 'wrong' | ''>('')

  useEffect(() => { document.title = 'Àrìa Learn — Code Quest Level 2' }, [])

  const handleDrag = (item: string) => setSelected(item)
  ...
  const saveProgress = async () => {
    ...
    try {
      ...
      await supabase.from('profiles').update({
        ...
        badges: [...new Set([...((prof?.badges as string[]) || []), 'drum_coder'])],
